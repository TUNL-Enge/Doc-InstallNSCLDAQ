...
proc Menu_ui {root args} {
	...
	label $base.label#1 \
		-text {ssh localhost before starting Menu.}

	label $base.label#2 \
		-text {Acquisition and Analysis}

	button $base.button#2 \
		-command "StartProc $base.button#2 \$BinDir/startReadout Readout $base.output" \
		-text Readout

	button $base.button#8 \
		-command "StartProc $base.button#8 \$BinDir/startScaler  Scaler  $base.output " \
		-text {Scaler Display}

	button $base.button#3 \
		-command "StartProc $base.button#3 \$BinDir/startSpecTcl SpecTcl" \
		-text SpecTcl
	...
	button $base.button#4 \
		-command "StartProc $base.button#4 \ico Shaper $base.output" \
		-text {no action}

	button $base.button#5 \
		-command "StartProc $base.button#5 \$BinDir/startCfd Cfd $base.output" \
		-text {V812 cfd}

	button $base.button#6 \
		-command "StartProc $base.button#6 \ico GDG $base.output" \
		-text {no action}
